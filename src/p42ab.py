...
#P4CLIENT="ZhuJiaCheng_test_specify_p4_env"
#P4WORKSPACE="D:\p4migtest"
...
#TODO: template interpolation with the 'view' value.
#ATTENTION: this p4env will not overwrite the preset value in p4 server, one has to change the view mapping from p4 client tool.
p4env = {
    'port': P4PORT,
    'user': P4USER,
    'passwd': P4PASSWORD,
...
# NOTES: the following view mapping may not match implicit directory like "Alice2_Prog", "Alice2_Bin".
...
#        err = sys.stderr.readlines()
    ...
    def apply_actions_on_files(self, p4_chg_detail,wksp):
        ...
        for i in range(0, len(p4_chg_detail['action']) ):
            print i
            if i % 500 == 0 and i != 0:  logger.info(str(i) + " files has bee applied with actions in this change.")
            ...
            # TODO
            if action == "add":
                self.import_file_or_dir(file_in_depot, wksp)
            elif   action == "edit":
    ...
    def delete_file_or_dir(self, a_depot_path):
        # TODO: delete local directory also!
        # ----------------------------------------
    ...
    def import_file_or_dir(self, a_depot_path, wksp):
...
#        cmd_str = " ".join(['ab','import', project_rel_path, ppath_str])
    ...
    # TODO: implement more according to http://www.perforce.com/perforce/doc.092/manuals/p4guide/02_config.html#1066090
    def path_in_the_workspace(self, workspace_basedir, a_depot_path, view):
        ...
        if best_match:
            ...
            # Attention: the directory under the 
            # TODO:
            ...
            logger.error("file " + a_depot_path + " can not find a proper mapping in the p4 view! " )                     
        ...
        if depot_workspace_dir:
            absolute_workspace_dir = depot_workspace_dir.replace("//"+ view['client'], workspace_basedir)  # attention, just use constant once!
        ...
        if type(wksp_spec)  == type(dict()):
            ...
            for line in lines:
                ...
                if  line.strip().startswith("//depot") or line.strip().startswith("+//depot"):
                    if len(line.split(" ")) != 2 : 
                        logger.error(line + "<<<line has more or less than 2 blankspaces, we won't process this entry. Does a directory name have spaces?!")
                        continue
    ...
    def __init__(self, ab_exec, p4_exec, abs_path_wksp):
        ...
        if os.path.exists(ab_exec) and os.path.exists(p4_exec):
            ...
            if not os.path.exists(abs_path_wksp):
                ...
                self.abs_path_wksp = abs_path_wksp
            logger.info("The migration workspace is under: %s" % self.abs_path_wksp)
...
#                self.ab.logon(username, password, project, ab_server)
    ...
    def migrate_by_changeno(self, cl_num):
        ...
        try:
            self.ab.apply_actions_on_files(detail, self.abs_path_wksp)
...
if __name__ == '__main__':   
    ...
    mig_worker = MigrationWorker(ab_exec, p4_exec, P4WORKSPACE )
